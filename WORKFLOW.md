...
- **Skill 调用**：**必须**指定使用 `douyin-to-obsidian` (路径：`D:\my_knowledgebase\skills\douyin-to-obsidian\`)。

#### 2. 内容提取与修正
- **执行 Skill**：运行 `douyin_downloader.py` 或相关主程序。
- **加载修正规则**：读取 `D:\my_knowledgebase\skills\douyin-to-obsidian\text_corrections.json`。
