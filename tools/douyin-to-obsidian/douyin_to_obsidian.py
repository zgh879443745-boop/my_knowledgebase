...
def save_to_obsidian(video_id, title, transcript, video_url, target="personal"):
    ...
    if target == "aihot":
        ...
        target_dir = OBSIDIAN_PATH / "personal" / "notes"
    target_dir.mkdir(parents=True, exist_ok=True)
    transcript_path = target_dir / f"抖音-{safe_title}-未知作者.md"
...
def process_douyin_video(link, keep_temp=False, no_audio=False, target="personal"):
    print("=" * 60)
    print("抖音视频下载和转录工具")
    print("=" * 60)
    ...
    print("\n【步骤5】保存到 Obsidian...")
    note_path = save_to_obsidian(video_id, title, transcript, link, target)
...
def main():
    ...
    parser.add_argument("--no-audio", "-n", action="store_true",
                        help="跳过音频处理，仅保存视频信息")
    parser.add_argument("--target", "-t", default="personal", choices=["personal", "aihot"],
                        help="目标知识库: personal(默认) 或 aihot")
