...
| 要求 | 说明 |
|------|------|
| **标签不可通用** | 禁止仅用 `[抖音, 视频笔记, 2026]` 等无区分度的标签。必须包含具体主题词（如 `Codex, Agent, Skills管理`） |
| **related 必填** | 至少填写 1-2 条关联笔记的 `[[标题]]`，无关联可填 `[[MOC-{主题}]]` |
| **正文含双链** | 核心概念首次出现时使用 `[[wikilink]]`（如 `[[Agent]]`、`[[Codex]]`） |
| **关联 MOC** | 笔记创建后，确认对应 MOC 页面已包含本条笔记的入口（触发规则见下方 [MOC 创建规则](#-moc-创建规则)） |
...
| 场景 | 操作 |
|------|------|
| 新笔记主题命中已有 MOC（Agent / Codex / 浏览器自动化 / AI学习路径 等） | 直接追加到对应 MOC |
| 新笔记主题**不命中**任何现有 MOC | 先存入对应子目录，暂不建 MOC |
| 同一新主题**累积 ≥2 篇**笔记，且所有现有 MOC 均无法覆盖 | **自动创建**新 MOC（命名 `MOC-{主题}.md`），回链该主题下所有相关笔记 |

> 新 MOC 创建后，同步更新 `index.md` 🗺️ 主题索引（MOC）段落。
